...
//Jason Cho Project 5 LinkedList.cpp
//implementation for invertRest private function and invert public function that calls invertRest
...
{
	if(k <= 0) 
		//basecase there's nothing to rotate
	{
		return;
	}
	else
	{
		Node<T>* cur_ptr = getNodeAt(item_count_-1);
			//sets the node pointer at the final position of the list
		cur_ptr->setNext(head_ptr_);
		head_ptr_ = cur_ptr;
			//assign node to last position
		getNodeAt(item_count_-1)->setNext(nullptr);
			//by setting to null, able to rotate
		rotate(k-1);
	}
...
void LinkedList<T>::invertRest(Node<T>* current_first_ptr)
{
	...
	else if(current_first_ptr->getNext() == nullptr)
	//basecase if recursive steps are done and current_first_ptr points to all nullptr
	{
		head_ptr_ = current_first_ptr;
		//head_ptr_ will be the last node which will be set at the front of the list.
		return;
	}
	...
	else
	{
		...
		//creates the reverse link
		current_first_ptr->setNext(nullptr);
		//disconnects the original link
	}
}
