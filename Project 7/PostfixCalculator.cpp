...
std::string PostfixCalculator::convertToPostfix(std::string infix_expression)
{
	...
	for(int i = 0;i < infix_expression.length();i++)
	{
		if(operatorCheck(infix_expression[i]) || isdigit(infix_expression[i])
	...
	else
	{
		for(int i = 0;i < infix_expression.length();i++)
		{
			...
			{
				postfix_result += infix_expression[i];
				while(isdigit(infix_expression[++i]))
				{
					postfix_result += infix_expression[i];//if number has more than one integer 
				}
				postfix_result += ' ';
			}
			...
			//Case: operator
			if(operatorCheck(infix_expression[i]))
			{
				...
				&& prescedence(infix_stack.top(),infix_expression[i]))
				{
